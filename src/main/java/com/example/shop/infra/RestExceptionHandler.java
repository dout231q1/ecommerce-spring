...
public class RestExceptionHandler {
    ...
    private ResponseEntity<ErrorResponse> notValidURLhandler(NoResourceFoundException exception){
        ErrorResponse error = new ErrorResponse(HttpStatus.BAD_REQUEST.value(), "Invalid URL '" + exception.getResourcePath() + "'. Please check the endpoint." );
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(error);
    }
    ...
    private ResponseEntity<ErrorResponse> methodArgumentTypeMismatchHandler(MethodArgumentTypeMismatchException exception) {
        ErrorResponse error = new ErrorResponse(HttpStatus.BAD_REQUEST.value(),  "'" + exception.getValue() + "' for parameter '" +  exception.getName() + "'. Excepted a valid number.'");
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(error);
    }
}
